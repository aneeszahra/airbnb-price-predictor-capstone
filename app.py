...
if not os.path.exists('model.pkl'):
    st.error("Model file model.pkl not found in the current directory.")
    st.stop()

if not os.path.exists('scaler.pkl'):
    st.error("Scaler file scaler.pkl not found in the current directory.")
    st.stop()

# -------------------------------------------------------------------
# Load model and scaler with pickle (version-stable)
try:
...
if st.button("Predict Price", type="primary", use_container_width=True):
    try:
        ...
        st.markdown(f"""
        <div style="background-color: #f0f2f6; padding: 30px; border-radius: 15px; text-align: center; border: 2px solid #2ecc71;">
            <h1 style="color: #2ecc71; font-size: 48px; margin: 0;">${prediction:.2f}</h1>
            <p style="color: #666; font-size: 16px; margin-top: 10px;">
                Estimated price per night for your listing
            </p>
            <p style="color: #888; font-size: 12px; margin-top: 5px;">
                Based on similar listings in {neighbourhood}
            </p>
        </div>
